...
#' @param group Factor name for centroid grouping. It will add dot colors.
#' @param group2 Factor name for extra grouping. It will add dot shapes. Defaults to NULL (no additional classification).
...
ggpca <- function(pca, pdata, fit = NULL, group, group2 = NULL, axes = c(1,2), centr = FALSE){
  ...
  if(!is.null(fit)){
    ...
    p_plot <- ggplot(uscores1, aes_string(x =colnames(comp1), y = colnames(comp2), col = group)) +
      geom_point(size = 3, aes(shape=!!shape_expr)) +
