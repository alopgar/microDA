...
Ellipse_PCA <- function(pca, phenot, factor, fcolors, comps = c(1,2), title = TRUE, level = NULL){
  ...
  plot(pca.coords, type = "p", asp = 1, cex.main=1.5,
       ...
       pch = 19, col = fcolors[as.numeric(as.factor(classes))])
  ...
  CIplot_biv(pca$x[,comps[1]], pca$x[,comps[2]], group = as.factor(classes), groupcols = fcolors, add = T, shade = T,
             shownames = F, showcentr = T)
}
